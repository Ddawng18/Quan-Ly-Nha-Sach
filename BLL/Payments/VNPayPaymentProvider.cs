...
public class VNPayPaymentProvider : IPaymentProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var content = new FormUrlEncodedContent(
                parameters.Concat([new KeyValuePair<string, string>("vnp_SecureHash", signature)]));
            ...
            if (responseCode == "00")
            {
                var transactionId = responseParams["vnp_TransactionNo"] ?? orderId;
            ...
            return new PaymentCreationResult(false, orderId, string.Empty, message);
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            FileLogger.Error($"VNPay CreatePayment failed: orderId={orderId}", ex);
            return new PaymentCreationResult(false, orderId, string.Empty, ex.Message);
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var responseParams = HttpUtility.ParseQueryString(responseText);
            var responseCode   = responseParams["vnp_ResponseCode"] ?? "";
            var txnStatus      = responseParams["vnp_TransactionStatus"] ?? "";

            var status = (responseCode, txnStatus) switch
            {
            ...
            var errorMessage = status == PaymentStatus.Paid ? null : responseParams["vnp_Message"];
